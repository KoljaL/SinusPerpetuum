...
## 3. Physik der Rollbewegung und EM-Beschleunigung

### 3.1 Kinematische Kopplung über die echte Bahngeschwindigkeit ($s$)
Zur Vermeidung mathematischer Artefakte berechnet die Simulation die Bewegung konsequent als eindimensionale Bewegung entlang der Raumkurve (Bogenlänge $s$). Die Kräfte greifen tangential an der Kurve an. Erst nach erfolgter Integration der Bahngeschwindigkeit ($v$) wird die Koordinate über ein numerisches Linienintegral in die kartesischen Bildschirm-Koordinaten ($x, y$) übersetzt.

### 3.2 Das Trägheitsmoment und die Beschleunigungskorrektur
Da die Kugel zweigleisig gelagert ist, rollt sie auf dem kleineren, effektiven Abrollradius $r_{\mathrm{eff}} \approx 1{,}64\,\text{cm}$ ab. Dadurch erhöht sich die Winkelgeschwindigkeit im Vergleich zum Abrollen auf einer flachen Ebene drastisch. Der Korrekturfaktor für die lineare Beschleunigung lautet:

$$
f_{\mathrm{acc}} = \frac{1}{1 + 0{,}4\left(\frac{R}{r_{\mathrm{eff}}}\right)^2} \approx 0{,}539
$$

Die resultierende Beschleunigung entlang der Bahnkurve berechnet sich somit aus der Summe aller tangentialen Kräfte dividiert durch die Masse, skaliert mit diesem Trägheitsfaktor: $a = f_{\mathrm{acc}} \cdot \frac{\sum F}{m}$.

### 3.3 Dynamische Normalkraft unter Berücksichtigung der Fliehkraft
Aufgrund der stark wellenförmigen Geometrie der Schienen erfährt die Kugel beim Durchfahren der Radien erhebliche Zentrifugalkräfte. Die für die Reibung maßgebliche Normalkraft $F_N$ ist daher geschwindigkeitsabhängig und wird über den lokalen Krümmungsradius $R_k$ der Kurve berechnet:

$$
F_N = m \cdot g \cdot \cos(\alpha) + \frac{m \cdot v^2}{R_k}
$$

In den Wellentälern ($R_k > 0$) erhöht die Fliehkraft den Anpressdruck und damit die Rollreibung drastisch; auf den Hügelkuppen ($R_k < 0$) wird die Kugel entlastet.

### 3.4 Das Haftreibungskriterium (Stiction)
Unterschreitet die Bahngeschwindigkeit einen Grenzwert ($|v| < 0{,}01\,\text{m/s}$) und ist die Summe aus Hangabtriebs- und Magnetkraft kleiner als die maximale Haftreibungskraft ($F_H = \mu_H \cdot F_N$), wird die Kugel mathematisch vollständig arretiert ($v = 0$). Ein unendliches, mikroskopisches Rutschen bei flachen Winkeln wird dadurch physikalisch korrekt unterbunden.

---

## 4. Elektromagnetisches Booster-System (Coils & Hallsensoren)
...
### 4.1 Feldcharakteristik der spitz zulaufenden Eisenkerne
Die integrierten Elektromagnete (12 mm Eisenkern, 1.018 Windungen bei 12 V Betriebsspannung) arbeiten tief im Bereich der magnetischen Sättigung ($B_{\text{sat}} \approx 1{,}8\,\text{Tesla}$). Durch die spitz zulaufenden Überstände wird der magnetische Fluss extrem konzentriert. Die resultierende Anziehungskraft auf die Stahlkugel folgt einer aggressiven Kelvin-Gradientengleichung:

$$
F_{\text{Mag}}(x) = F_{\text{max}} \cdot \frac{r_0^4}{(x + r_0)^4}
$$

Diese Charakteristik ist empirisch auf eine effektive Reichweite von $4\,\text{cm}$ auf der waagerechten Ebene kalibriert ($F \approx 0{,}05\,\text{N}$ bei $x = 4\,\text{cm}$), schnellt im unmittelbaren Nahbereich vor der Spitze ($x \to 0$) jedoch auf bis zu $22{,}5\,\text{Newton}$ hoch.

### 4.2 Wired-OR Sensorsteuerung und Software-Timer
